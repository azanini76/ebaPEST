...
!********  subroutine bpi_bpi_init_algorithmic_CVs
       ...
       ! INITIALIZATIONS
            ...
            BL(1)%keywords = (/'structural_conv','phi_conv','eba_conv',   &
            & 'it_max_structural','it_max_phi','it_max_eba','linesearch', &
...
!********  subroutine bpi_init_prior_mean_DATA
       ...
       ! INITIALIZATIONS
            ...
            nullify(d_PM%Partrans) !MD Vector of parameter transformation [0] None [1] Log [2] Power [3] Bounded
            nullify(d_PM%alpha)    !MD Exponent of power transformation of parameters
            nullify(d_PM%limits)   !MD Upper and Lower limits of bounded transformation
...
!*********  subroutine bpi_init_algorithmic_DATA(d_A,npar,nobs)
       ...
       allocate(d_A%beta_hat(p))  !Allocation 
       d_A%beta_hat = UNINIT_REAL
