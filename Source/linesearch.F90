module linesearch
 ...
 subroutine lns_proc(d_XQR,d_S,cv_PAR,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,cv_A,it_phi,miostruc,errstruc,lnsunit)
 ...
 implicit none
        ...
        integer ,             intent(in)    :: it_phi, lnsunit        
        ...
        start(1)=log((start(1)-limit(1))/(limit(2)-start(1)))
        step(1) =log(((limit(2)-0.1)-limit(1))/(limit(2)-(limit(2)-0.1)))-start(1)
...
10        format('Warning: Maximum number of iterations exceeded in linesearch procedure during quasi-linear iteration',i4, & 
        ...
        xmin(1)=exp(xmin(1))*limit(2)/(1+exp(xmin(1)))+limit(1)/(1+exp(xmin(1)))

        d_PAR%pars=xmin(1)*d_PAR%pars+(1-xmin(1))*d_PAR%pars_old 
        d_PAR%phi_T=ynewlo
        !!!write (lnsunit,*) 'final',xmin(1), ynewlo
 end subroutine lns_proc
 
end module linesearch 

 
   real ( kind = 8 )function Jmin (rho,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit, miostruc,errstruc) 
   ...
   integer lnsunit
   !Anti-Transform
   rho_ph=exp(rho)*limit(2)/(1+exp(rho))+limit(1)/(1+exp(rho))

   
   ! -- INITIALIZATION 
   d_PAR%pars_lns=rho_ph*d_PAR%pars+(1.-rho_ph)*d_PAR%pars_old 
