...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine bpo_write_banner()
      write(6,*)
      write(6,*) '*******************************************'
      write(6,*) '*          Welcome to ebaPEST             *'
      write(6,*) '*******************************************' 
      write(6,*) '    Version 1.0beta  October 19, 2018     '
      write(6,*) '*******************************************'
      write(6,*)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine bpo_write_nocmd()     
      write(6,*) 'ebaPEST is run using the command :-'
      write(6,*) 'ebaPEST ebapestfile '
      write(6,*) 'where ebapestfile is a ebaPEST control file'
    ...
    if (ci95_flag .eq. 1) then  ! only calculate LCL and UCL if posterior covariance was calculated
      ...
      if (maxval(d_PM%Partrans).ge.1) then  !If yes, the parameters transformation is required  
        do i = 1,cv_PAR%npar 
          ...
          elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then
            finalparvalue(i)=log((d_PAR%pars(i)-d_PM%limits(d_PAR%BetaAssoc(i),1))/(d_PM%limits(d_PAR%BetaAssoc(i),2)-d_PAR%pars(i)))
          endif
        enddo   
      endif
      ...
      if (maxval(d_PM%Partrans).ge.1) then  !If yes, the parameters back-transformation is required  
        do i = 1,cv_PAR%npar 
         ...
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then
              ucl(i) = ((exp(ucl(i))*d_PM%limits(d_PAR%BetaAssoc(i),2))+d_PM%limits(d_PAR%BetaAssoc(i),1))/(1+exp(ucl(i)))
              lcl(i) = ((exp(lcl(i))*d_PM%limits(d_PAR%BetaAssoc(i),2))+d_PM%limits(d_PAR%BetaAssoc(i),1))/(1+exp(lcl(i)))
         endif
        enddo 
      endif      
...
10  format('          ebaPEST RUN RECORD: CASE ', 1A)
!!! dimensions
...
!!! Algorithmic control variables
    ...
    write(bprunit,20) indent,'Outermost EBA Convergence'
    write(bprunit,35) indent,indent,'eba_conv: ',cv_A%bga_conv    
    write(bprunit,20) indent,'Maximum Number of Structural Parameter Iterations'
    ...
    write(bprunit,20) indent,'Maximum Number of Outermost EBA Iterations'
    write(bprunit,45) indent,indent,'it_max_eba: ',cv_A%it_max_bga       
...
57  format(1A,I4, ' Beta Associations were defined:')
    ...
    if (cv_A%Q_compression_flag .ne. 0)  then
        ...
        do i = 1,cv_PAR%p
            ...
            write(bprunit,20) indent,'Estimat. space for this Beta Assoc: [none] physical space, [log] log transf., [power] power transf., [bounded] bound transf.'
            select case (d_PM%Partrans(i))
              ...
              case (3)
                write(bprunit,21) indent,indent,'bounded - Estimation is in a bounded space',' with lower_limit ', d_PM%limits(i,1) , ' and upper_limit ', d_PM%limits(i,2)
            end select
...
!!! Structural Parameter Definitions
    ...
    do i = 1,cv_PAR%p
        ...
        select case (cv_S%var_type(i))
         ...
         case (3)
            write(bprunit,73) indent, indent, 'gaussian'
            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
            write(bprunit,74) indent, indent, indent, 'variance', d_S%theta_0(i,1)
            write(bprunit,74) indent,  indent,indent, 'correlation length', d_S%theta_0(i,2)
            
         case (4)
            write(bprunit,73) indent, indent, 'spherical'
            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
            write(bprunit,74) indent, indent, indent, 'variance', d_S%theta_0(i,1)
            write(bprunit,74) indent,  indent,indent, 'correlation length', d_S%theta_0(i,2)
...
21 format(4A, ' = ', 1ES13.6,1x,1A,' = ',1ES13.6)
...
!!! Parameter definitions
    ...
    call bpc_openfile(cparunit,trim(trim(casename) // '.epp.0'),1) ![1] at end indicates open with write access
    call bpo_write_allpars(cv_PAR,d_PAR,cparunit)
    close(cparunit)
    write(bprunit,85) indent,'For initial parameter values see file :- ', trim(trim(casename) // '.epp.0')
85  format(3A) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    write(bprunit,*) 
    write(bprunit,35) indent,' RES : ',d_PAR%phi_T
    write(bprunit,35) indent, 'Obj. Fun. : ',d_PAR%phi_M
...
!            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
           ...
           ! write(bprunit,174) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (1)
            write(bprunit,173) indent, indent, 'linear'
!            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
            write(bprunit,174) indent,  indent,indent, 'slope', d_S%theta(i,1)
           ! write(bprunit,177) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (2)
...
!            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
           ...
           ! write(bprunit,177) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (3)
            write(bprunit,173) indent, indent, 'gaussian'
!            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
            write(bprunit,174) indent, indent, indent, 'variance', d_S%theta(i,1)
            write(bprunit,174) indent,  indent,indent, 'correlation length', d_S%theta(i,2)
          !  write(bprunit,177) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (4)
            write(bprunit,173) indent, indent, 'spherical'
!            write(bprunit,25) indent, indent, 'Initial structural parameter values:'
            write(bprunit,174) indent, indent, indent, 'variance', d_S%theta(i,1)
            write(bprunit,174) indent,  indent,indent, 'correlation length', d_S%theta(i,2)
          !  write(bprunit,177) indent,  indent, 'ABIC', cv_S%str_obj_fun
    ...
    if (d_S%sig_opt .eq. 1) then
     ...
     write(bprunit,177)   indent, 'ABIC', cv_S%str_obj_fun
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do i = 1,cv_PAR%p
        ...
        select case (cv_S%var_type(i))
         case (0)
           ...
           ! write(bprunit,277) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (1)
         ...
         case (2)
           ...
           ! write(bprunit,277) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (3)
            write(bprunit,273) indent, indent, 'gaussian'
            write(bprunit,274) indent, indent, indent, 'variance', d_S%theta(i,1)
            write(bprunit,274) indent,  indent,indent, 'correlation length', d_S%theta(i,2)
          !  write(bprunit,277) indent,  indent, 'ABIC', cv_S%str_obj_fun
         case (4)
            write(bprunit,273) indent, indent, 'spherical'
            write(bprunit,274) indent, indent, indent, 'variance', d_S%theta(i,1)
            write(bprunit,274) indent,  indent,indent, 'correlation length', d_S%theta(i,2)
          !  write(bprunit,277) indent,  indent, 'ABIC', cv_S%str_obj_fun
        end select          
    enddo ! i = 1,cv_PAR%p
      ...
      write(bprunit,276) indent, indent, d_S%sig
       write(bprunit,277)   indent, 'ABIC', cv_S%str_obj_fun
