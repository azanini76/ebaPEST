...
!********  subroutine bdp_read_data_prior_mean (BL,cv_PM,d_PM,inunit,errmsg)
       ...
       ! DECLARATIONS
         ...
         allocate (d_PM%beta_0(cv_PAR%p))          
            d_PM%beta_0 = 0 ! Important to be 0! an array
            
   if (cv_PM%betas_flag.eq.0) then      
      call bdp_alloc_Partrans(d_PM,columnname,columnstring,5,BL(3)%numrows)
       !**********************************************************************************
       !*************     PARAMETER TRANSFORMATION FLAG     ******************************
         do j=1,BL(3)%numrows
           ...
           tmp_str = trim(adjustl(columnstring(2)))  
            ...
            if (tmp_str.eq.'none') then
                d_PM%Partrans(j)=0
            elseif (tmp_str.eq.'log') then
                d_PM%Partrans(j)=1
            elseif (tmp_str.eq.'power') then
                d_PM%Partrans(j)=2
            elseif (tmp_str.eq.'bounded') then
                d_PM%Partrans(j)=3
            else
              write(retmsg,60) j        
60            format('Error: Parameter transformation type not allowed.' &
            ...
            if (columnstring(4).ne.' ') call drealread(ifail, columnstring(4), d_PM%limits(j,1))
            if (columnstring(5).ne.' ') call drealread(ifail, columnstring(5), d_PM%limits(j,2))
         enddo
       !********************************************************************************
       !********************************************************************************
...
61           format('Error: Cannot provide prior beta values without covariance.' &
          ...
          case (1)    !covariance of beta on diagonal
            ...
            call bdp_alloc_d_PM(d_PM,columnname,columnstring,7,BL(3)%numrows)
            do i=1,BL(3)%numrows
              call ids_read_block_table(ifail,BL(3)%label,7,columnname,columnstring,line,filename)
...
600                  format('Error: Parameter transformation type not allowed.' &
              ...
              if (columnstring(4).ne.' ') call drealread(ifail, columnstring(4), d_PM%limits(i,1))
              if (columnstring(5).ne.' ') call drealread(ifail, columnstring(5), d_PM%limits(i,2))
              call drealread(ifail, columnstring(6), d_PM%beta_0(i))
              call drealread(ifail, columnstring(7), d_PM%Qbb(i,i))
            enddo
...
66                  format('Error: Parameter transformation type not allowed.' &
              ...
              call drealread(ifail, columnstring(6), d_PM%beta_0(i))
              do j = 7,numcol
                call drealread(ifail, columnstring(j), d_PM%Qbb(i,j-6))
              enddo
            enddo
        end select
    ...
    !********  subroutine bdp_read_data_parameters(BL,d_PAR,npargp,inunit,retmsg)
       ...
       ! ALLOCATIONS, READING, and PARSING  
         ! parameter data  and associated allocations
            ...
            allocate(d_PAR%prior(cv_PAR%npar))
        ...
        do k=1,nrows
          call ids_read_block_table(ifail,BL(4)%label,numcol,columnname,columnstring,line,filename)
             ...
             select case (cv_S%var_type(k))
                ...
                case(3) ! gaussian variogram
                    cv_S%num_theta_type(k) = 2
                case(4) ! spherical variogram
                    cv_S%num_theta_type(k) = 2
             end select
...
10         format('Warning: No structural parameter optimization requested. it_max_eba in algorithmic_cv block must be 1.',& !| no structural 
...
!********  subroutine bdp_read_data_observations(BL,d_OBS,nobsgp,nobs,inunit,retmsg,miostruc)
   ...
   ! ALLOCATIONS, READING, and PARSING
     ! observation data and associated allocations
        ...
        allocate(d_OBS%linobs(cv_OBS%nobs))
        allocate(d_OBS%misfit(cv_OBS%nobs))        
        ...
        d_OBS%linobs    = UNINIT_REAL ! array
        d_OBS%misfit    = UNINIT_REAL ! array        
...
!********  subroutine bdp_alloc_d_PM
       ...
       ! DECLARATIONS
         ...
         columnstring(3)=' '
         columnname(4)='lower_limit'
         columnstring(4)=' '
         columnname(5)='upper_limit'
         columnstring(5)=' '
         columnname(6)='beta_0'
         columnstring(6)=' '
