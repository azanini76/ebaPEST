...
! ************************************************
! ***      THIS SOFTWARE IS A MODIFICATION     ***
! ***       OF bgaPEST (Implementation of      ***
! ***      Bayesian Geostatistical inverse     *** 
! ***            method in PEST)               ***
! ***     https://pubs.usgs.gov/tm/07/c09/     ***
! *** https://github.com/mnfienen-usgs/bgaPEST ***
! ***                                          ***
! ***     The implemented mehod couples        ***
! ***  an empirical Bayesian approach and      ***
! ***    the Akaike's Bayesian Information     ***
! ***      Criterion (ABIC) for parameter      ***
! ***               estimation                 ***
! ***                                          ***
! ***            Marco D'Oria, PhD             ***
...
! ***            Andrea Zanini, PhD            ***
! ***       UNIVERSITY OF PARMA, ITALY         ***
! ***        andrea.zanini@unipr.it            ***
! ***                                          ***
! ***  Vers. 1.0beta --- October 28, 2018      ***
! ************************************************
...
!--  Main Data Arrays for OBS and PARS and ALGORITHM   
       integer                      :: n1, i, j
       integer                      :: forward_flag_der, ci95_flag,munit,kk
       ...
       integer                      :: ifail, restart, outunit, bprunit, cparunit, cobsunit, finalparunit, postcovunit,lnsunit
       ...
       double precision             :: delta_obs2 !square difference between maximum and minimum observation values to be used in NRMSE
...
! -- An extension of ".ebp" is added to the ebaPEST control file if necessary.
      i=LEN_TRIM(CTLFILE)
      IF(i.GE.5) THEN
        ATEMP=CTLFILE(I-3:I)
        CALL UTL_CASETRANS(ctlfile,'lo')
        IF(ATEMP.NE.'.eba') THEN 
          CASENAME = CTLFILE
          CTLFILE(I+1:)='.eba'
        ELSE
...
!--  INITIALIZE MIO STRUCTURE SO IT CAN BE PASSED    
    if(mio_initialise(errstruc,miostruc).ne.0) then  !MD mio_initialise is an integer
     ...
     !!!!lnsunit = utl_nextunit()
     !!!!  open (lnsunit,file='lns_file.txt')

! open up the main output record file 
       bprunit = utl_nextunit()          
       call bpc_openfile(bprunit,trim(trim(casename) // '.ebr'),1) ![1] at end indicates open with write access
...
!-- INITIALIZE THE INVERSE MODEL
    !Make X, Q0, R0
    call bxq_make_X_Q0_R0(d_PAR,cv_S,d_S,cv_PAR,d_XQR,cv_A,d_OBS,cv_OBS%nobs,Q0_All,cv_PM,d_PM,d_ANI)
...
!!! --- Initialize outer loop convergence values
          cv_S%str_obj_fun = huge_val !Initialize structural obj. fun. (aka ABIC) to huge value
...
!-- Run forward model once to obtain the initial modeled observation vector (consistent with the initial set of parameters d_PAR%pars)  
    call bpf_model_run(errstruc, d_MOD, cv_PAR,d_PAR, cv_OBS, cv_A,  d_OBS, d_A, 0, miostruc)
    !-- CALCULATE MISFIT and delta_obs2
    d_OBS%misfit=(d_OBS%obs - d_OBS%h)
    curr_phi=0.
     do i = 1, cv_OBS%nobs
        curr_phi = curr_phi + d_OBS%misfit(i)**2
     enddo  
        !curr_phi_conv =2.*abs(curr_phi - d_PAR%phi_T)/(abs(curr_phi)+abs(d_PAR%phi_T))
    d_PAR%phi_T=curr_phi
    delta_obs2=(maxval(d_OBS%obs)-minval(d_OBS%obs))**2
    
    if (maxval(d_PM%Partrans).ge.1) then  !If yes, the parameters transformation is required  
        call prior_par_trans(cv_PAR, cv_OBS, d_PAR, d_A, d_PM) !Converting parameters in the estimation space (d_PAR%prior)
    else
        d_PAR%prior  = d_PAR%pars  
    endif

    do b_ind = 1, cv_A%it_max_bga  !*********************************************************************** (more external loop)
    
    !***************************************************************************************************************************  
    !************************************* FROM HERE THE PARAMETER ESTIMATION LOOP *********************************************
    !***************************************************************************************************************************
    
          !curr_phi_conv = huge_val !Initialize current quasi linear objective function convergence
          !d_PAR%phi_T   = huge_val !Initialize current quasi-linear objective function value
          ...
          do p_ind = 1, cv_A%it_max_phi !************************************************************* (first intermediate loop)
            ...
            if (maxval(d_PM%Partrans).ge.1) then  !If yes, the parameters transformation is required  
               call sen_par_trans(cv_PAR, cv_OBS, d_PAR, d_A, d_PM) !Converting sensitivity and parameters in the estimation space (d_PAR%pars_old)
            endif
          
            !-- CALCULATE LINEARIZED OBSERVATION MISFIT
            call  bmo_H_only_operations(d_A,cv_OBS,d_PAR,cv_PAR,d_PM,d_OBS)
            !d_OBS%misfit=(d_OBS%obs - d_OBS%h)
            d_OBS%linobs=d_OBS%misfit + d_A%Hsold - d_A%Hsprior
            
            !-- SOLVE FOR THE BEST ESTIMATE           
            ...
            call  bmo_solve_be(d_XQR, d_S, d_PM, cv_PAR, cv_OBS, d_OBS, d_A, d_PAR,cv_PM)
                       
            !!!!!-- PERFORM LINESEARCH IF REQUESTED
            !!!!!if (cv_A%lns_flag.eq.1) then  !If yes, we perform the linesearch procedure  
            !!!!!   call lns_proc(d_XQR,d_S,cv_PAR,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,cv_A,p_ind,miostruc,errstruc,lnsunit)
            !!!!!endif
            ...
            !Calculate misfit and objective function 
            d_OBS%misfit=(d_OBS%obs - d_OBS%h)
            curr_phi=0.
            do i = 1, cv_OBS%nobs
                curr_phi = curr_phi + d_OBS%misfit(i)**2
            enddo  
            !!!curr_phi_conv =2.*abs(curr_phi - d_PAR%phi_T)/(abs(curr_phi)+abs(d_PAR%phi_T))
            curr_phi_conv = sqrt(curr_phi/cv_OBS%nobs/delta_obs2)
            d_PAR%phi_T=curr_phi
            d_PAR%phi_M=curr_phi_conv
            ...
            curr_par_file = trim(casename) // '.epp.' // trim(outer_iter) // '_' // trim(inner_iter)
            curr_resid_file = trim(casename) // '.ere.' // trim(outer_iter) // '_' // trim(inner_iter)
...
10              format('Warning: Maximum number of iterations exceeded in the parameter optimization loop during ebaPEST iteration',i4, & 
          ...
          !!!!!curr_bga_phi = curr_phi_conv  ! Set the current bga outer loop convergence equal to the current value of objective function
    !***************************************************************************************************************************  
    !******************************************** END OF PARAMETER ESTIMATION LOOP *********************************************
    !***************************************************************************************************************************
...
11           format('Warning: Maximum number of iterations exceeded in the parameter optimization loop during ebaPEST iteration',i4, & 
             ...
             if (b_ind.eq.1) then                                                                 
               d_OBS%linobs=d_OBS%misfit + d_A%Hsold - d_A%Hsprior    ! Just to have the last misfit consistent with the last pars (H is not recalculated, is consistent with sold)         
               call beg_ABIC(d_XQR,Q0_all,cv_OBS,d_OBS,cv_A,d_A,d_PAR,cv_S,d_S,d_PM,cv_PAR,cv_PM) !here the evaluation of ABIC with the initial parameters 
               prev_struct=cv_S%str_obj_fun                                                       !(only at the first eba loop)
             endif                                                              
               
             call ABIC_struct_param_optim(d_XQR,Q0_all,cv_OBS,d_OBS,cv_A,d_A,d_PAR,cv_S,d_S,d_PM,cv_PAR,cv_PM,b_ind,cv_S%num_theta_opt)
             curr_structural_conv=2.*(abs(prev_struct-cv_S%str_obj_fun))/(abs(cv_S%str_obj_fun)+abs(prev_struct))
             prev_struct = cv_S%str_obj_fun                           !Assign the current value of the objective function to the previous
             
             if (curr_structural_conv(1).le.cv_A%structural_conv) then !If yes, structural parameters have converged.
       ...
       !!!   call bpo_write_bpr_intermed_structpar(bprunit,cv_S,d_S,cv_PAR,d_PAR)
        ...
        if (cv_A%it_max_bga.ne.1) then    
         cv_S%struct_par_opt = 1
         call bpo_write_bpr_final_structpar(bprunit,cv_S,d_S,cv_PAR,d_PAR)
         cv_S%struct_par_opt = 0
         exit !If the struct. pars optimization is not required or struct. pars have converged (run the last parameter estimation), exit the loop
        endif
     endif
    !***************************************************************************************************************************  
    ...
    !!!!!!!*********************************************
    !!!!!!   curr_bga_conv =2.*(abs(prev_bga_phi - curr_bga_phi))/(abs(prev_bga_phi)+abs(curr_bga_phi))
    ...
    cobsunit = utl_nextunit()
    curr_resid_file = trim(casename) // '.ere.fin'
    call bpc_openfile(cobsunit,trim(curr_resid_file),1) ![1] at end indicates open with write access
    call bpo_write_residuals(cv_OBS,d_OBS,cobsunit)
    close(cobsunit) 
    ...
    !*************************************************************************************************************************
     if (cv_A%post_cov_flag.eq.1) then
      ...
      !-- write out the final parameter values and confidence intervals
        ...
        curr_par_file = trim(casename) // '.epp.fin'
