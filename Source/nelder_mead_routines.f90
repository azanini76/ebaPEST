...
contains
 ...
 subroutine nelmin_ls ( fn, n, start, xmin, ynewlo, reqmin, step, konvge, kcount,icount, numres,ifault, &
    & d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
...
!  Initial or restarted loop.
  do
    do i = 1, n
      p(i,nn) = start(i)
    enddo
    y(nn) = fn ( start,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
    icount = icount + 1
    do j = 1, n
      x = start(j)
      ...
      do i = 1, n
        p(i,j) = start(i)
      enddo
      y(j) = fn ( start,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
      icount = icount + 1
      start(j) = x
    enddo
...
!  Reflection through the centroid.
      ...
      enddo
      ystar = fn ( pstar,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
      icount = icount + 1
!  Successful reflection, so extension.
      if ( ystar < ylo ) then
        ...
        enddo
        y2star = fn ( p2star,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
        icount = icount + 1
!  Check extension.
          ...
          if ( y(ihi) < y2star ) then
            do j = 1, nn
              do i = 1, n
                ...
                xmin(i) = p(i,j)
              enddo
              y(j) = fn ( xmin,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
              icount = icount + 1
            enddo
            ylo = y(1)
            ilo = 1
...
!  Factorial tests to check that YNEWLO is a local minimum.
    ...
    do i = 1, n
      del = step(i) * eps
      xmin(i) = xmin(i) + del
      z = fn ( xmin,d_XQR,d_S,cv_PAR,cv_A,d_A,d_PAR,d_PM,cv_OBS,d_OBS,cv_PM,d_MOD,limit,lnsunit,miostruc,errstruc )
      icount = icount + 1
      if ( z < ynewlo ) then
        ifault = 2
        exit
...
!  Check to see if minimum reached.
      if ( icount <= kcount ) then
        jcount = konvge
        cnv=2*(abs(ylo-cv_S%str_obj_fun))/(abs(cv_S%str_obj_fun)+abs(ylo))
        cv_S%str_obj_fun=ylo
        if (cnv <= reqmin) then
            exit
        endif
!        z = 0.0D+00
