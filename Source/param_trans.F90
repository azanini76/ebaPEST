...
contains
  ...
  !Second s_old = log(s_old) !After s_old is in the estimation space
       ...
       do i = 1,cv_PAR%npar 
         ...
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then !Bounded transformation
           d_PAR%pars_old(i) = log((d_PAR%pars(i)-d_PM%limits(d_PAR%BetaAssoc(i),1))/(d_PM%limits(d_PAR%BetaAssoc(i),2)-d_PAR%pars(i)))
           do j = 1, cv_OBS%nobs
             d_A%H(j,i)=d_A%H(j,i)*((exp(d_PAR%pars_old(i))*(d_PM%limits(d_PAR%BetaAssoc(i),2)-d_PM%limits(d_PAR%BetaAssoc(i),1)) )/((1+exp(d_PAR%pars_old(i)))**2))
           end do
         end if
       end do
  ...
  !We do s = exp(s) After this s is again in the physical space
       ...
       do i = 1, cv_PAR%npar
         ...
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then  !Bounded transformation
           d_PAR%pars(i) = ((exp(d_PAR%pars(i))*d_PM%limits(d_PAR%BetaAssoc(i),2))+d_PM%limits(d_PAR%BetaAssoc(i),1))/(1+exp(d_PAR%pars(i)))
         end if
       enddo
  ...
  !We do s = exp(s) After this s is again in the physical space
       ...
       do i = 1, cv_PAR%npar
         ...
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then  !Bounded transformation
           d_PAR%pars_lns(i) = ((exp(d_PAR%pars_lns(i))*d_PM%limits(d_PAR%BetaAssoc(i),2))+d_PM%limits(d_PAR%BetaAssoc(i),1))/(1+exp(d_PAR%pars_lns(i)))
         endif
       enddo
  ...
  subroutine prior_par_trans(cv_PAR, cv_OBS, d_PAR, d_A, d_PM)
  !Subroutine to transform the parameters in the estimation space.
  !Only if required and for the parameters that require transformation. 
  !LOG CASE:
       implicit none
       ...
       do i = 1,cv_PAR%npar 
         if (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.1) then
             d_PAR%prior(i) = log(d_PAR%pars(i))   !MD At the beginning pars is the vector of the initial values of the parameters 
                 ! as read in the file. Then became the best estimate. Here we transform in the estimation space if required
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.2) then !Power transformation
           d_PAR%prior(i) = d_PM%alpha(d_PAR%BetaAssoc(i))*((d_PAR%pars(i)**(1./d_PM%alpha(d_PAR%BetaAssoc(i))))-1)
         elseif (d_PM%Partrans(d_PAR%BetaAssoc(i)).eq.3) then !Bounded transformation
           d_PAR%prior(i) = log((d_PAR%pars(i)-d_PM%limits(d_PAR%BetaAssoc(i),1))/(d_PM%limits(d_PAR%BetaAssoc(i),2)-d_PAR%pars(i)))
        end if
       end do
       
  end subroutine prior_par_trans
