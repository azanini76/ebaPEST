...
subroutine form_post_covariance(d_XQR, cv_PAR, cv_OBS, cv_S, cv_A, d_A, d_PAR,Q0_All,cv_PM,d_PM,d_S,VV,V)
  ...
  case(0) !Full Q0 matrix         
         ...
         do i=1, cv_PAR%npar      !Loop over all the parameters
           select case (cv_S%var_type(d_PAR%BetaAssoc(i)))
            ...
            case (2) ! means exponential ---> we need theta1 and theta2
              ...
              enddo
            case (3) ! means exponential ---> we need theta1 and theta2
              do j=i, cv_PAR%npar
                if (d_PAR%BetaAssoc(i).eq.d_PAR%BetaAssoc(j)) then !Search in the parameters list the associated parameters  
                   VV(i,j)=d_S%theta(d_PAR%BetaAssoc(i),1)*exp(-(d_XQR%Q0(i,j)**2)/(d_S%theta(d_PAR%BetaAssoc(i),2)**2))
                   VV(j,i)=VV(i,j) ! Because Qss is symmetric
                endif   
              enddo
            case (4) ! means Spherical ---> we need theta1 and theta2
              do j=i, cv_PAR%npar
                if (d_PAR%BetaAssoc(i).eq.d_PAR%BetaAssoc(j)) then !Search in the parameters list the associated parameters  
                   if (d_XQR%Q0(i,j).le.d_S%theta(d_PAR%BetaAssoc(i),2)) then
				       VV(i,j)=d_S%theta(d_PAR%BetaAssoc(i),1)*(1-1.5*(d_XQR%Q0(i,j)/d_S%theta(d_PAR%BetaAssoc(i),2))+0.5*((d_XQR%Q0(i,j)/d_S%theta(d_PAR%BetaAssoc(i),2)))**3)
                       VV(j,i)=VV(i,j) ! Because Qss is symmetric
                   endif  
                endif   
              enddo     
    ...
    !*****************************************************************************************************
    ! Make Qyy = H*Qss*Ht + sig*R0  and calculate the inverse
    !*****************************************************************************************************
    ...
    !*****************************************************************************************************
    !************* Make VV = Qss - QssHt*Qyy^-1*HQss (Qss is in VV and QssHt is in d_A%Qyy) **************
    !*****************************************************************************************************
    allocate(TMP(cv_PAR%npar,cv_OBS%nobs)) !Calculate QssHt*Qyy^-1
      call dgemm('n', 'n', cv_PAR%npar, cv_OBS%nobs, cv_OBS%nobs, 1.D0, d_A%Qsy, &
          cv_PAR%npar, d_A%Qyy, cv_OBS%nobs, 0.D0,  TMP, cv_PAR%npar)
      !now multiply QssHtQyy^-1 (TMP) by (QssHt)t, change the sign and add the result to Qss, forming VV
               ...
               case(0) !Means no Toeplitz.....just compressed form. Q0(p) is the full matrix for the p-th beta
                 ...
                 do it=1,Q0_All(p)%npar
                  do j=it,Q0_All(p)%npar
                    TMP(it,j)= exp(-Q0_All(p)%Q0_C(it,j)/(d_S%theta(Q0_All(p)%BetaAss,2)))
                    TMP(j,it)=TMP(it,j)
                  enddo
          ...
          case (3) ! means Gaussian ---> we need theta1 and theta2. We have 2 option: Toeplitz or not
             select case (Q0_All(p)%Toep_flag) !Selection of Toeplitz [1] or not [0]
               case(0) !Means no Toeplitz.....just compressed form. Q0(p) is the full matrix for the p-th beta
                 ...
                 do it=1,Q0_All(p)%npar
                  do j=it,Q0_All(p)%npar   
                   TMP(it,j)= exp(-(Q0_All(p)%Q0_C(it,j)**2)/((d_S%theta(Q0_All(p)%BetaAss,2)**2)))
                   TMP(j,it)=TMP(it,j)
                  enddo
                  V(start_v:end_v) = TMP(it,it)*(d_S%theta(Q0_All(p)%BetaAss,1)) !Only diagonal values of Qss
                 enddo
          ...
          case (4) ! means Spherical ---> we need theta1 and theta2. We have 2 option: Toeplitz or not
             select case (Q0_All(p)%Toep_flag) !Selection of Toeplitz [1] or not [0]
               case(0) !Means no Toeplitz.....just compressed form. Q0(p) is the full matrix for the p-th beta
                 ...
                 do it=1,Q0_All(p)%npar
                   do j=it,Q0_All(p)%npar
                    if (Q0_All(p)%Q0_C(it,j).le.d_S%theta(Q0_All(p)%BetaAss,2)) then
                      TMP(it,j)= 1-1.5*(Q0_All(p)%Q0_C(it,j)/d_S%theta(Q0_All(p)%BetaAss,2))+0.5*(Q0_All(p)%Q0_C(it,j)/d_S%theta(Q0_All(p)%BetaAss,2))**3
                      TMP(j,it)=TMP(it,j)
                    else 
                      TMP(it,j)=0.
                      TMP(j,it)=TMP(it,j)
...
!*****************************************************************************************************
subroutine toep_mult_post(Q0_All,ip,d_A,nobs,theta_1,theta_2,Lmax,Qsy,V,start_v,end_v, gaus_spher_flag)
